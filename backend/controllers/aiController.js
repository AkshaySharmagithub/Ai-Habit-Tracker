...
export const weeklyReport = async (req, res) => {
    try {
        ...
        if (!ctx.perHabit.length) {
            return res.json({
                content:
                    "You don't have any active habits yet. Create your first habit to start tracking — I'll generate a weekly report once there is data.",
            });
        }
...
export const suggestHabits = async (req, res) => {
    try {
        ...
        const userMsg = `User goals: ${goals || "not provided"}\nMost productive time: ${productiveTime || "not provided"
            }\nStruggles: ${struggles || "not provided"}\n\nReturn only valid JSON in this exact shape: {"suggestions":[{"name":"","description":"","frequency":"daily","category":"","icon":"","reason":""}]}`;
